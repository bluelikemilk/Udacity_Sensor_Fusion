...
void Proximity(int idx, const std::vector<std::vector<float>> points, std::vector<int> &cluster, std::vector<bool> &visited, KdTree* tree, float distanceTol){
	// find all connected componenet within distance tolarence
	// mark point as processed
	visited[idx] = true;
	// add point to cluster
	cluster.push_back(idx);
	// do iteration to all nearby points
	std::vector<int> nearbyIdx = tree->search(points[idx], distanceTol);
	// iterate through each nearby point
	for(int i:nearbyIdx){
		// if point has not been processed, do Proximity
		if(visited[i] == false) Proximity(i, points, cluster, visited, tree, distanceTol);
	}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	// iterate through each pointint idx=0; idx<points.size(); idx++
	for(int idx=0; idx<points.size(); idx++){
		
		if(visited[idx]) continue;
	
		// if not visited, create a cluster
		
		std::vector<int> cluster;
		// iteratively find all connected componenet
		Proximity(idx, points, cluster, visited, tree, distanceTol);
...
int main ()
{
    ...
    for (int i=0; i<points.size(); i++) 
  	...
  	std::cout << "Test Search clusters" << std::endl;
