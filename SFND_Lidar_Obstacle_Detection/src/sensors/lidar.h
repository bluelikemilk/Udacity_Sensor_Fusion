...
struct Lidar
{
	...
	Lidar(std::vector<Car> setCars, double setGroundSlope) // constructor of struct Lidar
		: cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0,0,2.6) // :cloud() is initializer list, when create a lidar object, it initialize cloud that will evetually store the Lidar scan
	{
		// TODO:: set minDistance to 5 to remove points from roof of ego car
		minDistance = 5;
		maxDistance = 50;
		resoultion = 0.2;
		// TODO:: set sderr (std of error) to 0.2 to get more interesting pcd files
		sderr = 0.2;
		cars = setCars;
